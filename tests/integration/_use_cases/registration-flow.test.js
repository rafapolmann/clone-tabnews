...
describe("Use case: Resgitration Flow (all successful", () => {
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@rafapolmann.dev.br>");
    expect(lastEmail.recipients[0]).toBe("<registration.flow@curso.dev>");
    expect(lastEmail.subject).toBe("Ative seu cadastro no FinTab!");
    expect(lastEmail.text).toContain("RegistrationFlow");

    activationTokenId = orchestrator.extractUUI(lastEmail.text);
  ...
  test("Get user information", async () => {
    const userReponse = await fetch("http://localhost:3000/api/v1/user", {
      headers: {
        cookie: `session_id=${createSessionsResponseBody.token}`,
      },
    });

    expect(userReponse.status).toBe(200);

    const userReponseBody = await userReponse.json();

    expect(userReponseBody.id).toBe(createSessionsResponseBody.user_id);
  });
});
